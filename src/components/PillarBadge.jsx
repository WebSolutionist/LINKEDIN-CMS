...
export default function PillarBadge({ pillar, size = 'sm', className = '' }) {
  ...
  const sizing = size === 'lg' ? 'text-xs px-3 py-1' : 'text-[11px] px-2.5 py-0.5';
  return (
    <span className={`inline-flex items-center gap-1.5 rounded-full font-medium border whitespace-nowrap shrink-0 ${base} ${sizing} ${className}`}>
